...
public class Marker implements Comparable<Marker> {
	...
	public Marker(String name, double latitude, double longitude, double altitude, int color,String address,float rating) {
		set(name, latitude, longitude, altitude, color,address,rating);
	}

	public synchronized void set(String name, double latitude, double longitude, double altitude, int color,String address,float rating) {
	...
	public synchronized String getVicinity(){
		return this.address;
	}
    ...
    protected synchronized void drawText(Canvas canvas) {
	    ...
	    if (distance<1000.0) {
	    	if(name.length()>10)
	        textStr = name.substring(0, 10) + " ("+ DECIMAL_FORMAT.format(distance) + "m)";    
	    	else
	    		  textStr = name + " ("+ DECIMAL_FORMAT.format(distance) + "m)";    
	    } else {
	        double d=distance/1000.0;
	        if(name.length()>10)
	        textStr = name.substring(0, 10)+ " (" + DECIMAL_FORMAT.format(d) + "km)";
	        else
	        	 textStr = name + " ("+ DECIMAL_FORMAT.format(distance) + "m)";    
	    }
	    ...
	    if (textBox==null) 
	    	textBox = new PaintableBoxedText(textStr, 30, 280);
	    else 
	    	textBox.set(textStr,30, 280);
